...
class EfficientOccupancyGridMapper(Node):
    ...
    def __init__(self):
        ...
        # Pose topics
        self.pose_topic_template = self.declare_parameter('pose_topic_template', '/robot_{id}/robot_pose_slam').value
        # If incoming PoseStamped is already in the global/map frame, skip applying robot_transforms
        self.poses_in_map_frame = bool(self.declare_parameter('poses_in_map_frame', True).value)
        ...
        for robot_id in self.robot_ids:
            self.create_subscription(
                PoseStamped,
                self.pose_topic_template.format(id=robot_id),
    ...
    def create_pose_callback(self, robot_id):
        ...
        def callback(msg):
            ...
            with self.lock:
                ...
                if self.poses_in_map_frame:
                    self.robot_headings[robot_id] = local_yaw
                else:
