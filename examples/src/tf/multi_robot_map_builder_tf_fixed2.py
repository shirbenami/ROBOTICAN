...
class OccupancyGridBuilderTF(Node):
    ...
    def __init__(self):
        ...
        self.tf_use_latest = bool(self.declare_parameter("tf_use_latest", True).value)
        ...
        self.origin_offset = float(self.declare_parameter("origin_offset", -1.0).value)

        if self.origin_offset < 0.0:
            # Auto: half the map size in meters
            self.origin_offset = 0.5 * self.grid_size * self.resolution
        self.get_logger().info(f"[map_builder] origin_offset={self.origin_offset:.3f} m (grid_size={self.grid_size}, res={self.resolution})")
        # Point filtering (after TF transform into map frame)
        self.z_min = float(self.declare_parameter("z_min", -1000000.0).value)
        self.z_max = float(self.declare_parameter("z_max", 1000000.0).value)
    ...
    def cloud_cb(self, msg: PointCloud2):
        ...
        self.get_logger().debug(f"TF OK map<-{msg.header.frame_id}: trans=({tf.transform.translation.x:.2f},{tf.transform.translation.y:.2f},{tf.transform.translation.z:.2f})")
